...
bot.on('message', async msg => {
  ...
  if (text === '/start' && isAllowed) {
    console.log(JSON.stringify(mainKeyboard))
    Promise.all([
      bot.sendMessage(chatId, `🤙Привет ${msg.from.first_name}, что интересует?`, JSON.stringify(mainKeyboard))
    ])
      .then((result) => {
...
bot.on('callback_query', async msg => {
  ...
  if (data === 'getprice') {
    console.log(msgID)
    bot.editMessageReplyMarkup(chatId, msgID, 'ТЕКСТ', JSON.stringify(mainKeyboard))
    // await bot.sendMessage(chatId, `👇Выбери тариф:`, rateKeyboard);
    // await bot.editMessageReplyMarkup(chatId, msgID, rateKeyboard)
  }
